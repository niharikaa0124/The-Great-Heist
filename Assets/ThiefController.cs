...
public class ThiefController : MonoBehaviour
{
    ...
    IEnumerator MoveTo(Vector3 destination)
    {
        ...
        if (endlineTilemap.HasTile(currentCell))
        {
            ...
            // 🔊 Play losing/escape audio
            if (LosingBeatAudio != null)
            {
                LosingBeatAudio.Play();
            }

            // 📺 Show escape panel and message
            if (escapePanel != null)
            {
        ...
        // ✅ Detect if stuck in a loop (e.g., A-B-A)
    ...
    void OnCollisionEnter2D(Collision2D collision)
    {
        if (collision.gameObject.CompareTag("Detective"))
        {
            ...
            if (caughtPanel != null)
            {
                ...
                if (caughtText != null && ScoreManager.instance != null)
                {
                    caughtText.text = "Thief caught in " + ScoreManager.instance.moveCount + " moves!";
                }
